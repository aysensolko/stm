...
var (
	botC         *telego.Bot
	botHandlerC  *th.BotHandler
	adminIdsC    []int64
	isRunningC   bool
	hostnameC    string
	hashStorageC *global.HashStorage
)

type LoginStatusClient byte

const (
	LoginSuccessC        LoginStatusClient = 1
	LoginFailC           LoginStatusClient = 0
	EmptyTelegramUserIDC                   = int64(0)
)
...
func (t *TgbotClient) NewTgbot() *TgbotClient {
	return new(TgbotClient)
}

func (t *TgbotClient) I18nBot(name string, params ...string) string {
	return locale.I18n(locale.Bot, name, params...)
}

func (t *TgbotClient) GetHashStorage() *global.HashStorage {
	return hashStorageC
}

func (t *TgbotClient) Start(i18nFS embed.FS) error {
	...
	hashStorageC = global.NewHashStorage(20 * time.Minute)
	...
	if tgBotID != "" {
		for _, adminID := range strings.Split(tgBotID, ",") {
			...
			adminIdsC = append(adminIdsC, int64(id))
		}
	}
	...
	botC, err = t.NewBot(tgBotToken, tgBotProxy, tgBotAPIServer)
	if err != nil {
...
func (t *TgbotClient) NewBot(token string, proxyUrl string, apiServerUrl string) (*telego.Bot, error) {
...
func (t *TgbotClient) IsRunning() bool {
	return isRunningC
}

func (t *TgbotClient) SetHostname() {
...
func (t *TgbotClient) Stop() {
	botHandlerC.Stop()
	botC.StopLongPolling()
	logger.Info("Stop Telegram receiver ...")
	isRunningC = false
	adminIdsC = nil
}

func (t *TgbotClient) encodeQuery(query string) string {
	// NOTE: we only need to hash for more than 64 chars
	if len(query) <= 64 {
		return query
	}

	return hashStorageC.SaveHash(query)
}

func (t *TgbotClient) decodeQuery(query string) (string, error) {
	if !hashStorageC.IsMD5(query) {
		return query, nil
	}

	decoded, exists := hashStorageC.GetValue(query)
	if !exists {
...
func (t *TgbotClient) OnReceive() {
	params := telego.GetUpdatesParams{
		Timeout: 10,
	}

	updates, _ := botC.UpdatesViaLongPolling(&params)

	botHandlerC, _ = th.NewBotHandler(botC, updates)

	botHandlerC.HandleMessage(func(_ *telego.Bot, message telego.Message) {
		t.SendMsgToTgbot(message.Chat.ID, t.I18nBot("tgbot.keyboardClosed"), tu.ReplyKeyboardRemove())
	}, th.TextEqual(t.I18nBot("tgbot.buttons.closeKeyboard")))

	botHandlerC.HandleMessage(func(_ *telego.Bot, message telego.Message) {
		t.answerCommand(&message, message.Chat.ID, checkAdminC(message.From.ID))
	}, th.AnyCommand())

	botHandlerC.HandleCallbackQuery(func(_ *telego.Bot, query telego.CallbackQuery) {
		t.answerCallback(&query, checkAdminC(query.From.ID))
	}, th.AnyCallbackQueryWithMessage())

	botHandlerC.HandleMessage(func(_ *telego.Bot, message telego.Message) {
		if message.UsersShared != nil {
			if checkAdminC(message.From.ID) {
...
func (t *TgbotClient) answerCommand(message *telego.Message, chatId int64, isAdmin bool) {
	...
	case "start":
		...
		if isAdmin {
			msg += t.I18nBot("tgbot.commands.welcome", "Hostname=="+hostnameC)
		}
...
func (t *TgbotClient) sendResponse(chatId int64, msg string, onlyMessage, isAdmin bool) {
	if onlyMessage {
...
func (t *TgbotClient) answerCallback(callbackQuery *telego.CallbackQuery, isAdmin bool) {
	...
	if isAdmin {
		...
		if len(dataArray) >= 2 && len(dataArray[1]) > 0 {
			...
			case "tgid_remove_c":
				...
				needRestart, err := t.inboundService.SetClientTelegramUserID(traffic.Id, EmptyTelegramUserIDC)
				if needRestart {
					t.xrayService.SetToNeedRestart()
				}
				if err == nil {
...
func checkAdminC(tgId int64) bool {
	return false
}

func (t *TgbotClient) SendAnswer(chatId int64, msg string, isAdmin bool) {
...
func (t *TgbotClient) SendMsgToTgbot(chatId int64, msg string, replyMarkup ...telego.ReplyMarkup) {
	if !isRunningC {
		return
	}
...
func (t *TgbotClient) SendMsgToTgbotAdmins(msg string, replyMarkup ...telego.ReplyMarkup) {
	if len(replyMarkup) > 0 {
		for _, adminId := range adminIdsC {
...
func (t *TgbotClient) SendBackupToAdmins() {
	if !t.IsRunning() {
		return
	}
	for _, adminId := range adminIdsC {
...
func (t *TgbotClient) sendExhaustedToAdmins() {
	if !t.IsRunning() {
		return
	}
	for _, adminId := range adminIdsC {
...
func (t *TgbotClient) getServerUsage(chatId int64, messageID ...int) string {
...
func (t *TgbotClient) sendServerUsage() string {
	info := t.prepareServerUsageInfo()
	return info
}

func (t *TgbotClient) prepareServerUsageInfo() string {
	info, ipv4, ipv6 := "", "", ""
	...
	info += t.I18nBot("tgbot.messages.hostname", "Hostname=="+hostnameC)
...
func (t *TgbotClient) UserLoginNotify(username string, password string, ip string, time string, status LoginStatusClient) {
	if !t.IsRunning() {
		return
	}
	...
	msg := ""
	if status == LoginSuccessC {
		msg += t.I18nBot("tgbot.messages.loginSuccess")
		msg += t.I18nBot("tgbot.messages.hostname", "Hostname=="+hostnameC)
	} else if status == LoginFailC {
...
func (t *TgbotClient) getInboundUsages() string {
	info := ""
	// get traffic
...
func (t *TgbotClient) getInbounds() (*telego.InlineKeyboardMarkup, error) {
	inbounds, err := t.inboundService.GetAllInbounds()
...
func (t *TgbotClient) getInboundClients(id int) (*telego.InlineKeyboardMarkup, error) {
...
func (t *TgbotClient) clientInfoMsg(
...
func (t *TgbotClient) getClientUsage(chatId int64, tgUserID int64, email ...string) {
...
func (t *TgbotClient) searchClientIps(chatId int64, email string, messageID ...int) {
...
func (t *TgbotClient) clientTelegramUserInfo(chatId int64, email string, messageID ...int) {
...
func (t *TgbotClient) searchClient(chatId int64, email string, messageID ...int) {
	traffic, err := t.inboundService.GetClientTrafficByEmail(email)
	if err != nil {
		logger.Warning(err)
		msg := t.I18nBot("tgbot.wentWrong")
...
func (t *TgbotClient) searchInbound(chatId int64, remark string) {
...
func (t *TgbotClient) getExhausted(chatId int64) {
	trDiff := int64(0)
	exDiff := int64(0)
...
func (t *TgbotClient) notifyExhausted() {
	trDiff := int64(0)
	exDiff := int64(0)
	now := time.Now().Unix() * 1000
			...
			if len(inbound.ClientStats) > 0 {
				...
				if err == nil {
					for _, client := range clients {
						if client.TgID != 0 {
							chatID := client.TgID
							if !int64ContainsC(chatIDsDone, chatID) && !checkAdminC(chatID) {
								var disabledClients []xray.ClientTraffic
								var exhaustedClients []xray.ClientTraffic
...
func int64ContainsC(slice []int64, item int64) bool {
	for _, s := range slice {
		if s == item {
			return true
		}
...
func (t *TgbotClient) onlineClients(chatId int64, messageID ...int) {
	if !p.IsRunning() {
		return
	}
...
func (t *TgbotClient) sendBackup(chatId int64) {
	...
	if err == nil {
		...
		_, err = botC.SendDocument(document)
		if err != nil {
...
func (t *TgbotClient) sendBanLogs(chatId int64, dt bool) {
	if dt {
...
func (t *TgbotClient) sendCallbackAnswerTgBot(id string, message string) {
	params := telego.AnswerCallbackQueryParams{
		CallbackQueryID: id,
		Text:            message,
	}
	if err := botC.AnswerCallbackQuery(&params); err != nil {
		logger.Warning(err)
	}
}

func (t *TgbotClient) editMessageCallbackTgBot(chatId int64, messageID int, inlineKeyboard *telego.InlineKeyboardMarkup) {
	params := telego.EditMessageReplyMarkupParams{
		ChatID:      tu.ID(chatId),
		MessageID:   messageID,
		ReplyMarkup: inlineKeyboard,
	}
	if _, err := botC.EditMessageReplyMarkup(&params); err != nil {
		logger.Warning(err)
	}
}

func (t *TgbotClient) editMessageTgBot(chatId int64, messageID int, text string, inlineKeyboard ...*telego.InlineKeyboardMarkup) {
	...
	if _, err := botC.EditMessageText(&params); err != nil {
		logger.Warning(err)
	}
}
